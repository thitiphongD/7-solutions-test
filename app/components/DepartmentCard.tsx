...
const DepartmentCard = ({
    ...
    return (
        <div className="rounded-lg border border-gray-300 overflow-hidden hover:shadow-lg transition-shadow duration-300">
            <div className="px-4 py-5 sm:p-6">
                ...
                <div className="mt-3">
                    <div className="flex items-center justify-between">
                        ...
                        <p className="text-sm font-medium text-gray-700">Post Code</p>
                    </div>
                    <div className="mt-1 max-h-32 overflow-y-auto">
                        {Object.entries(addressUser).map(([name, postal]) => (
                            <div key={name} className="flex justify-between text-sm">
                                <span className="text-gray-800 font-light text-sm">{name}</span>
                                <span className="text-gray-500 font-light text-sm">{postal}</span>
                            </div>
                        ))}
                    </div>
                </div>
